...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveGame() { const gameState = { player, corruptData, wave, currentRound, upgrades }; localStorage.setItem('eg_game3_state', JSON.stringify(gameState)); }
    function loadGame() {
    ...
    function startGame() { isCombatActive = true; spawnNextEnemy(); autoAttackInterval = setInterval(playerAutoAttack, 2000); setInterval(updateCooldowns, 1000); updateUI(); }
    ...
    function enemyAttack() {
        ...
        if (enemy.name === "EL KERNEL CORRUPTO" && Math.random() < 0.25) {
            ...
            enemy.hp = Math.min(allRoundsEnemies[1][3].hp, enemy.hp + healAmount);
    ...
    async function handleEnemyDefeat() {
        ...
        if (enemy.name === "REFLEJO PROGRAMADO") {
            ...
            dialogueOverlay.classList.remove('hidden');
            startFinalBattleBtn.classList.add('hidden'); // << CORRECCIÓN AÑADIDA AQUÍ
        ...
        if (wave > currentEnemies.length) {
            ...
            if (currentRound > allRoundsEnemies.length) {
                ...
                setTimeout(() => { roundOverlay.classList.add('hidden'); isCombatActive = true; spawnNextEnemy(); }, 5000);
            }
        } else {
            saveGame();
            setTimeout(() => { isCombatActive = true; spawnNextEnemy(); }, 2000);
        }
        updateUI();
    }
    ...
    function updateUI() {
        ...
        if (enemy.hp > 0) {
            ...
            enemyHpFill.style.width = `${(enemy.hp / originalEnemy.hp) * 100}%`;
            enemyHpText.textContent = `${Math.ceil(enemy.hp)} / ${originalEnemy.hp}`;
        } else {
